...
class MathGenerator:
    ...
    # --- 7. FRACTIONS & CONVERSIONS (Consolidated) ---
    
    # 7a. Fraction to Decimal
    ...
    def generate_geometry(self) -> List[Dict[str, Any]]:
        ...
        if shape == 'rectangle':
            ...
            svg = f"""<div style="display:flex; flex-direction:column; align-items:center;"><svg width="200" height="120" viewBox="0 0 200 120"><rect x="25" y="10" width="150" height="80" style="fill:{fill_color};stroke:{stroke_color};stroke-width:3" /><text x="100" y="110" font-family="Arial" font-size="14" text-anchor="middle">{w} m</text><text x="185" y="55" font-family="Arial" font-size="14" text-anchor="middle">{h} m</text></svg><div style="margin-top:10px;">Find the <b>{mode.title()}</b>.</div></div>"""
        elif shape == 'square':
            s = random.randint(4, 12)
            ans = (s * s) if mode == 'area' else (4 * s)
            svg = f"""<div style="display:flex; flex-direction:column; align-items:center;"><svg width="150" height="150" viewBox="0 0 150 150"><rect x="25" y="25" width="100" height="100" style="fill:{fill_color};stroke:{stroke_color};stroke-width:3" /><text x="75" y="145" font-family="Arial" font-size="14" text-anchor="middle">{s} m</text></svg><div style="margin-top:10px;">Find the <b>{mode.title()}</b>.</div></div>"""
        else:
            r = random.randint(3, 9)
            ans = round(3.14 * r * r, 2) if mode == 'area' else round(2 * 3.14 * r, 2)
            svg = f"""<div style="display:flex; flex-direction:column; align-items:center;"><svg width="150" height="150" viewBox="0 0 150 150"><circle cx="75" cy="75" r="50" stroke="{stroke_color}" stroke-width="3" fill="{fill_color}" /><line x1="75" y1="75" x2="125" y2="75" style="stroke:#000;stroke-width:2" /><text x="100" y="70" font-family="Arial" font-size="14" text-anchor="middle">r = {r} m</text></svg><div style="margin-top:10px;">Find the <b>{mode.title()}</b> (π=3.14).</div></div>"""
    ...
    # --- 9. DATA INTERPRETATION (SVG Charts) ---
    def generate_data_interpretation(self) -> List[Dict[str, Any]]:
        ...
        if q_type == 'max':
            ans = labels[values.index(max(values))]
            text = f"Which category has the highest {topic['unit']}?"
            inp_type = 'text'
        elif q_type == 'min':
            ans = labels[values.index(min(values))]
            text = f"Which category has the lowest {topic['unit']}?"
            inp_type = 'text'
        elif q_type == 'total':
            text = f"What is the total {topic['unit']} across all categories?"
            ans = sum(values)
            inp_type = 'single'
        else:
            text = f"What is the difference in {topic['unit']} between {labels[0]} and {labels[-1]}?"
        ...
        for i, (lbl, val) in enumerate(zip(labels, values)):
            ...
            y = 120 - h
            x = start_x + i * 60
            color = "#4caf50" if val == max(values) else "#2196f3"
    ...
    def generate_logical_reasoning(self) -> List[Dict[str, Any]]:
        ...
        if mode == 'series':
            ...
            text = f"Find the next number in the series: <b>{', '.join(display_seq)}</b>"
            return [{'id': self._unique_id(), 'question_text': text, 'type': 'single', 'correct_answer': ans, 'category': 'Logical Reasoning'}]
        else:
            word = random.choice(self.scenarios.get('lr_coding_words', ['APPLE']))
            ...
            text = f"If <b>{word}</b> is coded as <b>{coded}</b>, how is <b>{target}</b> coded?"
            return [{'id': self._unique_id(), 'question_text': text, 'type': 'text', 'correct_answer': target_coded, 'category': 'Logical Reasoning'}]

    # --- 11. DATA SUFFICIENCY (Text with Instructions) ---
    def generate_data_sufficiency(self) -> List[Dict[str, Any]]:
        problem = random.choice(self.scenarios.get('ds_problems', [
            {'question':'Find X', 'stat1':'X=1', 'stat2':'Y=2', 'correct':'Only I'}
        ]))
        ...
        <div class="text-start">
            ...
            <div class="card mb-3 bg-light border-0">
                <div class="card-body">
                    <p class="mb-2"><strong>Statement I:</strong> {problem['stat1']}</p>
                    <p class="mb-0"><strong>Statement II:</strong> {problem['stat2']}</p>
                </div>
            </div>
            <div class="alert alert-primary py-2 px-3 small">
                <i class="bi bi-info-circle-fill"></i> 
                <strong>Instructions:</strong> Determine sufficiency and type exactly one of these options:
                <br>
                <span class="badge bg-white text-dark border mt-1">Only I</span>
                <span class="badge bg-white text-dark border mt-1">Only II</span>
                <span class="badge bg-white text-dark border mt-1">Both</span>
                <span class="badge bg-white text-dark border mt-1">Neither</span>
            </div>
        </div>
        """
    ...
    def generate_profit_loss(self) -> List[Dict[str, Any]]:
        items = self.scenarios['profit_loss_items']
        names = self.scenarios['profit_loss_names']
        item = random.choice(items)
        name = random.choice(names)
        
        q_type = random.choice(['amount', 'percent'])
        if q_type == 'amount':
            cp = random.randint(5, 50) * 10
            is_profit = random.choice([True, False])
            if is_profit:
                profit = random.randint(1, 10) * 5
                sp = cp + profit
                text = f"{name} bought a {item} for ${cp} and sold it for ${sp}. What is the Profit?"
                ans = profit
            else:
                loss = random.randint(1, 10) * 5
                sp = cp - loss
                text = f"{name} bought a {item} for ${cp} and sold it for ${sp}. What is the Loss?"
                ans = loss
        else:
            cp = random.choice([100, 200, 300, 400, 500])
            perc = random.choice([10, 20, 25, 50])
            is_profit = random.choice([True, False])
            if is_profit:
                sp = cp + (cp * perc // 100)
                text = f"If CP = ${cp} and SP = ${sp}, what is the Profit %?"
                ans = perc
            else:
                sp = cp - (cp * perc // 100)
                text = f"If CP = ${cp} and SP = ${sp}, what is the Loss %?"
                ans = perc
        return [{
            'id': self._unique_id(),
            'question_text': text,
    ...
    def generate_unitary_method(self) -> List[Dict[str, Any]]:
        mode = random.choice(['cost', 'work'])
        if mode == 'cost':
            group_type = random.choice([('dozen', 12), ('score', 20), ('pack of 10', 10)])
            name, count = group_type
            item = random.choice(self.scenarios['unitary_cost_items'])
            unit_price = random.randint(2, 9)
            total = unit_price * count
            target = random.choice([2, 3, 5])
            ans = target * unit_price
            text = f"If 1 {name} of {item} costs ${total}, cost of {target}?"
        else:
            scen = random.choice(self.scenarios['unitary_work_scenarios'])
            actor, task = scen['actor'], scen['task']
            w1 = random.choice([5, 10, 15])
            d1 = random.choice([6, 12, 20])
            effort = w1 * d1
            ...
            ans = effort // w2
            text = f"If {w1} {actor} can {task} in {d1} days, how many days for {w2} {actor}?"
    ...
    def generate_all(self) -> List[Dict[str, Any]]:
        ...
        # 2 Questions per category (except Fractions which gets 6 questions total)
